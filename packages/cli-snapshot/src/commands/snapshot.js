...
export class Snapshot extends Command {
  ...
  async loadStaticSnapshots(pathname) {
  ...
  async loadSnapshotsFile(pathname) {
    let ext = path.extname(pathname);
    let config = {};

    // load snapshots file
    if (ext === '.js') {
      config = require(path.resolve(pathname));
      if (typeof config === 'function') config = await config();
    } else if (ext === '.json') {
      config = JSON.parse(fs.readFileSync(pathname, { encoding: 'utf-8' }));
    } else if (ext.match(/\.ya?ml$/)) {
      config = YAML.parse(fs.readFileSync(pathname, { encoding: 'utf-8' }));
    } else {
